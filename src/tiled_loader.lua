...
function lib.TiledMap_GetMapW () return gMapLayers.width end
function lib.TiledMap_GetMapH () return gMapLayers.height end

-- returns the mapwidth actually used by tiles
function lib.TiledMap_GetMapWUsed ()
	local maxx = 0
	local miny = 0
	local maxy = 0
...
function lib.TiledMap_GetNearestTileByTypeOnLayer (x,y,z,iTileType,maxrad)
	local w = lib.TiledMap_GetMapW()
	local h = lib.TiledMap_GetMapW()
	local maxrad2 = max(x,w-x,y,h-y) if (maxrad) then maxrad2 = min(maxrad2,maxrad) end
	if (lib.TiledMap_GetMapTile(x,y,z) == iTileType) then return x,y end
	for r = 1,maxrad2 do
		for i=-r,r do
			local xa,ya = x+i,y-r if (lib.TiledMap_GetMapTile(xa,ya,z) == iTileType) then return xa,ya end -- top
			local xa,ya = x+i,y+r if (lib.TiledMap_GetMapTile(xa,ya,z) == iTileType) then return xa,ya end -- bot
			local xa,ya = x-r,y+i if (lib.TiledMap_GetMapTile(xa,ya,z) == iTileType) then return xa,ya end -- left
			local xa,ya = x+r,y+i if (lib.TiledMap_GetMapTile(xa,ya,z) == iTileType) then return xa,ya end -- right
		end
	end
end

function lib.TiledMap_GetMapTile (tx,ty,layerid) -- coords in tiles
    local row = gMapLayers[layerid][ty]
    return row and row[tx] or kMapTileTypeEmpty
end

function lib.TiledMap_SetMapTile (tx,ty,layerid,v) -- coords in tiles
    local row = gMapLayers[layerid][ty]
...
function lib.TiledMap_ListAllOfTypeOnLayer (layerid,iTileType)
	local res = {}
	local w = lib.TiledMap_GetMapW()
	local h = lib.TiledMap_GetMapH()
	for x=0,w-1 do
	for y=0,h-1 do
		if (lib.TiledMap_GetMapTile(x,y,layerid) == iTileType) then table.insert(res,{x=x,y=y}) end
	end
	end
	return res
end

function lib.TiledMap_GetLayerZByName (layername) for z,layer in ipairs(gMapLayers) do if (layer.name == layername) then return z end end end
function lib.TiledMap_SetLayerInvisByName (layername) gTileMap_LayerInvisByName[layername] = true end

function lib.TiledMap_IsLayerVisible (z)
	local layer = gMapLayers[z]
	return layer and (not gTileMap_LayerInvisByName[layer.name or "?"])
end

function lib.TiledMap_GetTilePosUnderMouse (mx,my,camx,camy)
	return floor((mx+camx-love.graphics.getWidth()/2)/kTileSize),
			   floor((my+camy-love.graphics.getHeight()/2)/kTileSize)
end

function lib.TiledMap_DrawNearCam (camx,camy,fun_layercallback)
	...
	if (lib.TiledMap_IsLayerVisible(z)) then
    for x = minx,maxx do
    for y = miny,maxy do
        local gfx = gTileGfx[lib.TiledMap_GetMapTile(x,y,z)]
        if (gfx) then
...
function lib.TiledMap_Parse(filename)
    local xml = lib.LoadXML(love.filesystem.read(filename))
